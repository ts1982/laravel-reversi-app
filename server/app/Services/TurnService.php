...
readonly class TurnService {
    public function __construct(
        private GameResultRepository $game_result_repository,
    ...
    public function findLatestGameTurnByTurnCount(int $turn_count): array {
        $turn_entity = $this->turn_repository->findForTurnCount($turn_count);

        $game_result = null;
        if ($turn_entity->gameEnded()) {
            $game_result = $this->game_result_repository->findForGameId($turn_entity->getGameId());
        }
    ...
    public function registerTurn(Request $request): void {
        ...
        $previous_turn = $this->turn_repository->findForTurnCount($prev_turn_count);
        ...
        // ターンを保存する
        $this->turn_repository->save($new_turn);
        ...
        if ($new_turn->gameEnded()) {
            ...
            $this->game_result_repository->save($game_result_entity);
        }
    }
}
